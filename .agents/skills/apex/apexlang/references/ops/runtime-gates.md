...
4. Run validate-only by default with `node tools/apexctl.mjs runtime roundtrip --app-path <final_app_path> --db-connection-name <db_connection_name> --import-intent validate-only [--execution-mode auto|build-root|path]`.
5. Treat that runtime roundtrip as the `APEXLang source import` lane. It validates/imports the staged source tree and is not interchangeable with any separate compiled SQL export import path.
...
18. When export backups are produced, write them under `artifacts/apex-exports/<app>/`, never under `applications/<app>/`.
...
- Treat local repo validation as advisory helper work for the live roundtrip lane. Record whether it ran, failed, or was skipped by roundtrip policy, but do not block live validate/import solely on that result.
...
- Runtime export backups are durable artifacts, not generated app-root content. Keep them under `artifacts/apex-exports/<app>/` so `applications/<app>/` remains a clean APEXLang source tree.
- If a tool-generated export backup lands inside `applications/<app>/apex-exports`, relocate it before final validation or fail the run with a clear remediation message.
