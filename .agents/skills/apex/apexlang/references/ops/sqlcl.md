...
- Reuse `references/ops/reusable-prompts/prompt-normalization.md` when a workflow needs to normalize terse or fragmentary user input before domain execution.
...
- Packaged and source runs must execute through the same runtime executor rooted in `tools/runtime.mjs`. The packaged launcher may verify and cache the packaged runtime artifact, but it must not redefine runtime behavior.
...
- Treat any post-import runtime verification findings as diagnostics: record them in runtime artifacts, but do not rewrite a successful `import_status` or `runtime_gate_status` to fail.
  ...
  - when export backups are produced, place them under `artifacts/apex-exports/<app>/`, not inside `applications/<app>/`
