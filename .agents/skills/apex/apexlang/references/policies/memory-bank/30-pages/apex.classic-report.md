...
2. Main report region must use `type: classicReport` and follow the canonical Classic Report default template block owned by `references/policies/memory-bank/40-components/apex.templates.md`.
...
### Guidance
- Mirror `templates/page-examples/classic-report-page/classic-report-page._index.md` for structure, column ordering, and the canonical Classic Report default template block.
- Keep classic report `appearance.templateOptions` and `componentAppearance.templateOptions` to exact accepted values. `#DEFAULT#` stays standalone, report modifiers such as `t-Report--stretch` are separate entries, and documented composite values remain one atomic entry when the catalog/runtime lists them that way.
- Treat the `@/contextual-info` Classic Report family as the documented exception path. All other Classic Report generation must use the canonical default block from `references/policies/memory-bank/40-components/apex.templates.md`.
