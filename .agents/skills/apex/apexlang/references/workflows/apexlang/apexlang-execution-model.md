...
- `not-bundled/plsql` (local PL/SQL integration adapter; generic PL/SQL and utPLSQL authority stays with Oracle upstream DB skills)
- `not-bundled/schema-modeling` (schema modeling)

Router shortcut mapping (canonical ownership)
- APEX generation -> `SKILL.md`
- local PL/SQL maintenance + testing orchestration -> `not-bundled/plsql`
- Schema modeling -> `not-bundled/schema-modeling`
...
2. `SKILL.md` normalizes the request with the shared prompt-normalization reference and asks only one simple-English clarification round for critical blockers.
3. `SKILL.md` routes to the appropriate app-development subskill.
4. For full generation, `SKILL.md` first loads `assets/router-catalog.json`, `assets/load-policies.json`, and `assets/apexlang/catalog.json`, then opens the narrow matching README package plus `assets/apex-generation/components.registry.json` only when needed.
5. The execution subskill loads minimal rules using `memory-bank/rules-mapping.json`.
...
Hard gates
- Never read `.archive/`.
- Enforce guard precedence from `memory-bank/00-guard/`.
